...
- **A2A.** Two layers, by design:
  - **Open A2A surface (the interoperability mandate).** Every agent is published over the **open Agent-to-Agent protocol** via ADK's native `to_a2a()` adapter (`service/a2a_app.py`): a discoverable **Agent Card** at `/.well-known/agent-card.json` plus a **JSON-RPC 2.0** endpoint. Any A2A-speaking enterprise agent — ADK, LangGraph, CrewAI, or otherwise — can read the card and call the agent without bespoke glue. Reproducible end-to-end with `uv run python -m scripts.verify_a2a`, which uses the stock `a2a-sdk` client to discover the card and exercise the JSON-RPC endpoint (no GCP creds required). This is what satisfies "seamlessly discovered by and coordinate with other enterprise agents."
  - **Internal mesh transport.** Inside the deployed hub-and-spoke system, inter-agent calls run over Vertex AI Agent Engine's managed streaming channel (`async_stream_query` in `shared/a2a.py`), ID-token-authenticated per audience and OpenTelemetry-trace-propagated across hops. The two lateral edges (Listing Intake → Pricing for live anchors; Dispute Triage → Pricing for replay) are by design — they preserve auditability across the dispute flow Beat 2 demonstrates.
  - We deliberately do **not** use Vertex's managed `A2aAgent` wrapper: as of the build it is blocked by an upstream `vertexai` ↔ `a2a-sdk` version skew (documented in `OpenItems_B4.md` / commit `f941dd6`). ADK's framework-native `to_a2a()` is the supported path and needs no second runtime.
