...
- **`DEMO_MODE` gates the unauthenticated surface.** The `/demo/v1/*` shim + static UI force the demo partner and skip auth; `service/app.py` mounts them only when `DEMO_MODE=true`. **Deliberately ON for the public Fly app since 2026-07-25** (owner decision: the live demo is the product for now) — this means anyone with the URL spends Gemini tokens and writes rows under `demo_001`; there is no rate limiting. Before real customers rely on the `/v1` surface, either flip it off (`fly secrets unset DEMO_MODE -a surplusas-agents`) or move the demo to its own Fly app.
